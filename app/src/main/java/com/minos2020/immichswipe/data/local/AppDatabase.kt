...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
         ...
         * - On repart d'une table vide pour garantir la propreté des données (cache).
         */
        private val MIGRATION_8_9 = object : Migration(8, 9) {
            override fun migrate(db: SupportSQLiteDatabase) {
                AppLogger.i("Database", "Exécution Migration 8 -> 9 (Ajout userId à album_assets)")
                // 1. Supprimer l'ancienne table (on ne migre pas le cache car il est lié à l'utilisateur)
                db.execSQL("DROP TABLE IF EXISTS album_assets")
